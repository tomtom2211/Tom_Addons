...
public class TomsAddonsClient implements ClientModInitializer {
    ...
    public void onInitializeClient() {
        ...
        ClientCommandRegistrationCallback.EVENT.register((dispatcher, commandRegistryAccess) -> dispatcher.register(literal("status")
                            ...
                            try {
                                ...
                                if (player != null) {
                                    ...
                                    hypixelAPI.getStatus(UUID.fromString(formatUuid(uuid)))
                                            .thenAccept(result -> {
                                                final boolean status = result.getSession().isOnline();
                                                if (status){
                                                    final String currently = "Online";
                                                    if(!result.getSession().getServerType().getName().isBlank()&&!result.getSession().getMode().isBlank()) {
                                                        final String server = result.getSession().getServerType().getName();
                                                        final String lobby = result.getSession().getMode();
                                                        context.getSource().sendFeedback(Text.literal("%s is currently %s on %s in %s".formatted(nickname,currently,server,lobby)));
                                                    }
                                                    else{
